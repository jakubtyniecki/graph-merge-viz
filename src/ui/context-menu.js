...
export function setupContextMenu(panel) {
  ...
  panel.cy.on('cxttap', e => {
    ...
    show(pos.clientX || pos.x, pos.clientY || pos.y, items);
  });

  // Right-click on node
  panel.cy.on('cxttap', 'node', e => {
    const node = e.target;
  ...
  panel.cy.on('cxttap', 'edge', e => {
    ...
    const pos = e.originalEvent || e;
    show(pos.clientX || pos.x, pos.clientY || pos.y, [
      {
